...
// @import_models
import { Lead, Campaign, Third, User, Adnetwork, Tracking, Message, Brand, Interaction } from "@app/models"
import { twilioUtility } from "@core/utilities/messages/twilioUtility";
...
class LeadService {
  ...
  public async external (_params) {
    try{
      const brand = await Brand.findOne({_id: _params.brand}).lean()
      if(_params.number){
        try{
          twilioUtility.sendSMS({
            to_number: _params.number,
            message: `${brand.name}: Gracias por contactarnos en instantes uno de nuestros asesores se comunicara contigo.`,
            from_number: `default:sms`
          })
        } catch (error) {
          console.log('error', error)
        }
      ...
      if(_params.contact){
        ...
        third = await Third.findOne({number: _params.number, brand: _params.brand, type: 'cli'}).lean()
      }
      if(third){
        if(_params.content){
        ...
        const lead = _params.not_found ?? await Lead.findOne({brand:_params.brand, contact: third._id, status: {$in:['active', 'grading']}}).lean()
        if(lead){
          const tracking = await Tracking.findOne({brand: _params.brand, 'defaults.again': true}).lean()
          ...
          const adnetwork = await Adnetwork.findOne({brand: _params.brand, 'defaults.organic': true}).lean()
          const campaign = await Campaign.findOne({status: 'active', 'users.adnetworks': adnetwork._id, brand: _params.brand})
          .select('users.$')
          .lean()
        ...
        const adnetwork = await Adnetwork.findOne({brand:_params.brand, 'defaults.organic': true}).lean()
        const campaign = await Campaign.findOne({brand:_params.brand, status: 'active', 'users.adnetworks': adnetwork._id})
        .select('users.$')
        .lean()
